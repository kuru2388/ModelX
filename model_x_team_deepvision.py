...
print(f"  Dimensions: {data_frame.shape[0]:,} rows × {data_frame.shape[1]} columns")
...
print(f"\n✓ Strong predictors found: {len(strong_predictors)}")
if len(strong_predictors) > 0:
    for feature, corr_value in strong_predictors.items():
        print(f"  • {feature:<35} {corr_value:>7.3f}")
...
plt.axvline(x=0.3, color='red', linestyle='--', label='Strong threshold (±0.3)')
...
if 'BIRTHYR' in df_final.columns:
    ...
    print(f"✓ AGE calculated from BIRTHYR")
...
if 'AGE' in df_final.columns:
    ...
    print(f"✓ Age-based features created: age_squared, age_group")
...
if 'EDUC' in df_final.columns:
    ...
    print(f"✓ Education features: educ_group, college_educated, high_school_grad")
...
if all(var in df_final.columns for var in cardio_vars):
    ...
    print(f"✓ Cardiovascular risk count created")
...
if all(var in df_final.columns for var in metabolic_vars):
    ...
    print(f"✓ Metabolic risk count created")
...
if all(var in df_final.columns for var in mental_vars):
    ...
    print(f"✓ Mental health count created")
...
if all(var in df_final.columns for var in family_vars):
    ...
    print(f"✓ Family history count created")
...
print(f"✓ Lifestyle risk score created")
...
# Verify scaling (mean≈0, std≈1 for training)
...
if NEEDS_BALANCING or minority_pct < 40:
    ...
    print("✓ No balancing needed - classes are reasonably balanced")
...
print(f"\n✓ Best baseline model: {best_baseline} (ROC-AUC: {baseline_df.loc[best_baseline, 'ROC-AUC']:.4f})")
...
print(f"CV ROC-AUC: {selection_df.iloc[0]['CV_Mean_ROC_AUC']:.4f} ± {selection_df.iloc[0]['CV_Std']:.4f}")
...
print("✓ Model saved: best_dementia_model.pkl")
